  ...
  describe "PART 2" do
    ...
    describe "#num_near_matches" do
      ...
      it "should not include exact matches" do
        expect(code.num_near_matches(Code.new(["R", "G", "B", "B"]))).to eq(1)
        expect(code.num_near_matches(Code.new(["R", "R", "B", "B"]))).to eq(2)
        expect(code.num_near_matches(Code.new(["G", "R", "R", "R"]))).to eq(3)
      end
    end
